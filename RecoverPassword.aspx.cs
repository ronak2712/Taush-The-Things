...
public partial class RecoverPassword : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            GUIDvalue = Request.QueryString["Rid"];
            if (GUIDvalue != null)
            {
                ...
                if (dt.Rows.Count != 0)
                {
                    Rid = Convert.ToInt32(dt.Rows[0][1]);
                }
                else
                {
    ...
    protected void btRecPass_Click(object sender, EventArgs e)
    {
        if (tbNewPass.Text != "" && tbConfirmPass.Text != "" && tbNewPass.Text == tbConfirmPass.Text)
        {
            using (SqlConnection con = new SqlConnection(CS))
            {
                SqlCommand cmd = new SqlCommand("UPDATE Registration SET RPassword='" + tbNewPass.Text + "' where Rid='" + Rid + "'", con);
                con.Open();
                cmd.ExecuteNonQuery();
                SqlCommand cmd2 = new SqlCommand("delete from ForgotPassword where Rid='" + Rid + "'", con);
